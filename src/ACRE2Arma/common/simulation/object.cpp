...
acre::simulation::vertex::~vertex()
{
}


acre::simulation::face::face(
    const acre::p3d::face_p p3d_face,
    const acre::p3d::lod_p p3d_lod,
    const acre::p3d::model_p p3d,
    acre::simulation::lod *object_lod)
{
...
acre::simulation::face::face(uint32_t v1, uint32_t v2, uint32_t v3, acre::simulation::lod *object_lod) {
	this->type = 3;
...
void acre::simulation::vertex::animate(const glm::mat4 &matrix)
{
    acre::vector3<float> temp_vector = this->original_vertex;
    glm::vec4 temp_gl_vector = glm::vec4(temp_vector.x(), temp_vector.y(), temp_vector.z(), 1.0f);
    temp_gl_vector = matrix*temp_gl_vector;
    this->animated_vertex = acre::vector3<float>(temp_gl_vector.x, temp_gl_vector.y, temp_gl_vector.z);
}

acre::simulation::named_selection::named_selection(
    const acre::p3d::named_selection_p p3d_selection,
    const acre::p3d::lod_p p3d_lod,
    const acre::p3d::model_p p3d,
    acre::simulation::lod *object_lod)
{
...
void acre::simulation::named_selection::animate(const glm::mat4 &matrix)
{
...
acre::simulation::vertex_table::vertex_table(const acre::p3d::vertex_table_p p3d_vertex_table, const acre::p3d::lod_p p3d_lod, const acre::p3d::model_p p3d, bool reversed) : animated(false)
{
    this->vertices.resize(p3d_vertex_table->points.size);
	acre::vector3<float> center_off = p3d_lod->autocenter_pos;//p3d->info->cog_offset;//p3d->info->center_of_gravity + p3d->info->offset_2 + p3d->info->cog_offset;
    ...
    for (uint32_t i = 0; i <= p3d_vertex_table->points.size - 1; ++i) {
        if (p3d->info->autocenter) {
			acre::vector3<float> new_vertex = p3d_vertex_table->points[i]; 
...
acre::simulation::vertex_table::~vertex_table()
{
}

acre::simulation::lod::lod(const acre::p3d::lod_p p3d_lod, const acre::p3d::model_p p3d, bool reversed)
{
    this->id = p3d_lod->id;
    ...
    for (acre::p3d::face_p p3d_face : p3d_lod->faces) {
        this->faces.push_back(std::make_shared<face>(p3d_face, p3d_lod, p3d, this));
    }

    for (acre::p3d::named_selection_p p3d_selection : p3d_lod->selections) {
...
acre::simulation::lod::~lod()
{
}

acre::simulation::lod_animation_info::lod_animation_info(
    animation *_animation,
    lod_p _lod,
    const acre::p3d::animate_bone_p p3d_animate_bone,
    const acre::p3d::animation_p p3d_animation,
    const acre::p3d::model_p p3d,
	bool reversed) : animation_definition(_animation), lod(_lod)
{
    this->index = p3d_animate_bone->index;
    if (p3d->info->autocenter) {
		acre::vector3<float> center_off = _lod->autocenter_pos;//p3d->info->cog_offset;//p3d->info->center_of_gravity + p3d->info->offset_2 + p3d->info->cog_offset;
		//center_off.y(center_off.y() * 2);
		//center_off.x(0);
		//center_off.z(0);
		if (reversed) {
...
acre::simulation::animation::animation(object *parent_object, const acre::p3d::animation_p p3d_animation, const acre::p3d::model_p p3d, bool reversed)
{
    ...
    for (acre::p3d::animate_bone_p animation_bone : p3d_animation->bones) {
...
acre::simulation::animation::~animation()
{
}
...
animation_transform acre::simulation::animation::animate(const float phase, const std::vector<uint32_t> &lods, animation_transform base_transforms)
{
    ...
    for (auto lod_id : lods) {
        ...
        acre::vector3<float> rotation_offset = acre::vector3<float>(0, 0, 0);
...
float acre::simulation::animation::get_scale(float phase)
{
    float scale = 0;
...
acre::simulation::bone::bone(
    const std::string _name,
    const std::vector<std::string> &children,
    const std::map<std::string, acre::p3d::bone_p> &p3d_bones,
    bone *_parent,
    const acre::p3d::model_p p3d,
    object * sim_object
    ) : parent(_parent), name(_name), base_object(sim_object)
{
    for (auto const child_bone : children) {
        if (sim_object->all_bones.find(child_bone) == sim_object->all_bones.end()) {
            acre::p3d::bone_p p3d_bone = p3d_bones.find(child_bone)->second;
...
void acre::simulation::bone::animate(const std::map<std::string, float> &animation_state, const std::vector<uint32_t> &lods, animation_transform base_transforms)
{
...
acre::simulation::object::object()
{
}

acre::simulation::object::object(const acre::p3d::model_p model, bool reverse_ = false) : reversed(reverse_)
{
    for (acre::p3d::lod_p p3d_lod : model->lods) {
    ...
    for (acre::p3d::animation_p p3d_animation : model->animations) {
        this->animations.push_back(std::make_shared<animation>(this, p3d_animation, model, reversed));
    }

    std::map<std::string, acre::p3d::bone_p> p3d_bones;
...
void acre::simulation::object::animate(const std::map<std::string, float> &animation_state, const std::vector<uint32_t> &selected_lods)
{

...
if args.publish:
    ...
    # DEBUG
    do_action(["git", "status"], "Debug 1 error")
    do_action(["git", "branch"], "Debug 2 error")
    do_action(["git", "log", "--oneline", "-n20"], "Debug 3 error")

    do_action(["git", "pull"], "Failed to update target branch from 'origin/{}'".format(target_branch))

    # DEBUG
    do_action(["git", "status"], "Debug 4 error")

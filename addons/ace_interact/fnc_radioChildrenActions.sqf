...
if (!(_radio in ACRE_ACTIVE_EXTERNAL_RADIOS)) then {
    ...
    _actions pushBack [_action, [], _target];
    _action = ["acre_retrieve_radio", localize LSTRING(unshareRadio), "", {[(_this select 2) select 0, false] call EFUNC(sys_external,allowExternalUse)}, {[(_this select 2) select 0] call EFUNC(sys_external,isRadioShared)}, {}, _params]  call ace_interact_menu_fnc_createAction;
    _actions pushBack [_action, [], _target];
};

...
if (_target isKindOf "CAManBase") then {
    ...
    if (!isNull _vehicleInfantryPhone) then {
        ...
        private _action = ["acre_give_infantryTelephone", localize LSTRING(giveInfantryPhone), "", {[_player getVariable [QGVAR(vehicleInfantryPhone), nil], _target, 2, _player] call FUNC(updateInfantryPhoneStatus)}, {true}, {}, {}] call ace_interact_menu_fnc_createAction;
    ...
    if (vehicle acre_player != _target) then {
        ...
        if (isNull _vehicleInfantryPhone) then {
            ...
            if (acre_player == _unitInfantryPhone) then {
                ...
                private _action = ["acre_take_infantryTelephone", localize LSTRING(takeInfantryPhone), "", {[_target, _player, 1] call FUNC(updateInfantryPhoneStatus)}, {true}, {}, {}] call ace_interact_menu_fnc_createAction;
                _actions pushBack [_action, [], _target];
            };
        } else {
            if (_vehicleInfantryPhone == _target) then {
                // Generate the action to return the infantry telephone
                private _action = ["acre_return_infantryTelephone", localize LSTRING(returnInfantryPhone), "", {[_target, _player, 0] call FUNC(updateInfantryPhoneStatus)}, {true}, {}, {}] call ace_interact_menu_fnc_createAction;
                _actions pushBack [_action, [], _target];
            };
        };
    } else {

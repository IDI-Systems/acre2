 ...
 *
 * Example:
 * [1] call acre_sys_ws38_fnc_zoomChannelBlockSelector
 *
 * Public: No
 */
            ...
            if (_index < 0) then {
                if (_channelindex < _currentChannel || {_currentVolume != 3 && {_currentVolume != _volumeindex}}) then {
                    ...
                    if (_channelindex < _currentChannel) then {
                        ...
                        (MAIN_DISPLAY displayCtrl 106) ctrlSetText format ["\idi\acre\addons\sys_ws38\Data\knobs\channel\prc343_ui_pre_%1.paa", _channelindex + 1];
                    };
                    if (_volumeindex > _currentVolume) then {
                        DEC(_volumeindex);
                        _animationhandler set [2, _volumeindex];
                        (MAIN_DISPLAY displayCtrl 107) ctrlSetText format ["\idi\acre\addons\sys_ws38\Data\knobs\volume\prc343_ui_vol_%1.paa", _volumeindex];
                    };
                    if (_volumeindex < _currentVolume) then {
                        INC(_volumeindex);
                        _animationhandler set [2, _volumeindex];
                    ...
                    //(MAIN_DISPLAY displayCtrl 106) ctrlSetText format ["\idi\acre\addons\sys_ws38\Data\knobs\channel\%1\acre_prc343_channelKnob_%2.paa", (_currentChannel + 1), _index];
                    DEC(_index);

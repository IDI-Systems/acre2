...
if (_numSpokenLanguages > 1) then {
    ...
    [_languageName, "Now speaking", "", 1, EGVAR(sys_list,LanguageColor)] call EFUNC(sys_list,displayHint);
    ...
    if (_numSpokenLanguages == 1) then {
        ...
        ["No Babel Active", "", "", 1, EGVAR(sys_list,LanguageColor)] call EFUNC(sys_list,displayHint);
    };
};

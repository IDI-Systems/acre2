...
void acre::signal::model::Arcade::process(result *const result_, const glm::vec3 &tx_pos_, const glm::vec3 &rx_pos_, const std::string &rx_antenna_name, const float32_t frequency_Hz, const float32_t power_mW) {
    ...
    const float32_t ituLoss = 36.0f; /* base loss level (based on empirical testing...) */
    ...
    const float32_t lossModel = ituLoss + (static_cast<float32_t>(rand()) / (static_cast<float32_t>(RAND_MAX) + 1.0f) - 0.5f);

    /* Total Link Budget - SIGNAL STRENGTH */
    const float32_t linkBudget = txPower - lossTx - lossFreeSpace - lossModel - lossRx; /* Assume antenna gain is 0 for both*/

    result_->result_v = dbm_to_v(linkBudget, 50.0f);
    result_->result_dbm = linkBudget;
}

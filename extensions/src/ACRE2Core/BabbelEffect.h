...
public:
    ...
    void process(int16_t *const samples, const int32_t sampleCount) {
        ...
        buffer = new float32_t[sampleCount];
        floatPointer[0] = buffer;
        int16_t *tempSamples = new int16_t[sampleCount];
        memcpy(tempSamples, samples, sizeof(int16_t)*sampleCount);
        int16_t maxAmp = 1;
        for (int32_t i = 0; i < sampleCount; ++i) {
            samples[i] = (int16_t)(((float32_t)tempSamples[i]*1.0f)*cosf(((float32_t)i/(m_sfreq/m_mfreq))*1.4f*(float32_t) M_PI));
            //samples[i] += (short)(((float)tempSamples[i]*1.0f+offset)*cos(((float)i/(sfreq/500))*4*M_PI)+offset);
            if (abs(samples[i]) > maxAmp)
                maxAmp = samples[i];
            buffer[i] = (float32_t) samples[i]/(float32_t) SHRT_MAX;
        ...
        for (int32_t i = 0; i < sampleCount; ++i) {
            samples[i] = (int16_t)(floatPointer[0][i]*SHRT_MAX);

...
//AcreResult CFilterPosition::process(short* samples, int sampleCount, int channels, CPlayer *player, const unsigned int* channelSpeakerArray, unsigned int *channelMask, acre_volume_t volume) {
AcreResult CFilterPosition::process(short* samples, int sampleCount, int channels, const unsigned int speakerMask, CSoundMixdownEffect *params) {
    ...
    if (params->getParam("isWorld") == POSITIONAL_EFFECT_ISWORLD) {
        ...
        if (params->getParam("speakingType") == static_cast<float32_t>(AcreSpeaking::direct)) {
            /*if(CEngine::getInstance()->getSoundEngine()->getCurveModel() == AcreCurveModel::amplitude) {
            ...
            if (CEngine::getInstance()->getSoundEngine()->getCurveModel() == AcreCurveModel::selectableA) {
                Emitter.CurveDistanceScaler = 1.0f*(params->getParam("curveScale"));
                Emitter.pVolumeCurve = NULL;
            } else if (CEngine::getInstance()->getSoundEngine()->getCurveModel() == AcreCurveModel::selectableB) {
                Emitter.CurveDistanceScaler = 1.0f*(params->getParam("curveScale"));
    ...
    */
    ...
    if (CEngine::getInstance()->getSoundEngine()->getCurveModel() == AcreCurveModel::amplitude || CEngine::getInstance()->getSoundEngine()->getCurveModel() == AcreCurveModel::selectableA) {

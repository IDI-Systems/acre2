...
//acre_result_t CFilterPosition::process(short* samples, int sampleCount, int channels, CPlayer *player, const unsigned int* channelSpeakerArray, unsigned int *channelMask, acre_volume_t volume) {
acre_result_t CFilterPosition::process(short* samples, int sampleCount, int channels, const unsigned int speakerMask, CSoundMixdownEffect *params) {
    ...
    if (params->getParam("isWorld") == POSITIONAL_EFFECT_ISWORLD) {
        ...
        if (params->getParam("speakingType") == acre_speaking_direct) {
            /*if(CEngine::getInstance()->getSoundEngine()->getCurveModel() == acre_curveModel_amplitude) {
            ...
            if (CEngine::getInstance()->getSoundEngine()->getCurveModel() == acre_curveModel_selectableA) {
                Emitter.CurveDistanceScaler = 1.0f*(params->getParam("curveScale"));
                Emitter.pVolumeCurve = NULL;
            } else if (CEngine::getInstance()->getSoundEngine()->getCurveModel() == acre_curveModel_selectableB) {
                Emitter.CurveDistanceScaler = 1.0f*(params->getParam("curveScale"));
    ...
    */
    ...
    if (CEngine::getInstance()->getSoundEngine()->getCurveModel() == acre_curveModel_amplitude || CEngine::getInstance()->getSoundEngine()->getCurveModel() == acre_curveModel_selectableA) {

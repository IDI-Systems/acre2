...
CNamedPipeServer::CNamedPipeServer(const std::string fromPipeName, const std::string toPipeName) {
    ...
    while (!this->getShuttingDown()) {
        ...
        while (this->getConnectedWrite()) {
            ...
            if (this->m_sendQueue.try_pop(msg)) {
                if (msg) {
                    lastTick = clock() / CLOCKS_PER_SEC;
                    size = (uint32_t) strlen((const char *) msg->getData()) + 1;
                    if (size > 3) {
                        // send it and free it
                        //LOCK(this);
                        this->lock();
                        ret = WriteFile( 
                            ...
                            msg->getData(),              // message 
                            size,                        // message length 
                            (LPDWORD) &cbWritten,                  // bytes written 
                            NULL);                       // not overlapped 
                        this->unlock();
    ...
    */
    while (!this->getShuttingDown()) {
        ...
        while (this->getConnectedRead()) {
            ...
            ret = false;
            do {
                ret = ReadFile(this->m_pipeHandleRead, mBuffer, BUFSIZE, (LPDWORD) &cbRead, NULL);
